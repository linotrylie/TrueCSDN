<template>
    <div class="marked">
        <div ref="preview" class="write">
            <span v-dompurify-html="html" class=""></span>
            <el-image v-if="imgView" id="imgview" style="height:0px" :src="url" :preview-src-list="srcList">
            </el-image>
        </div>
            ...
            <div class="toc" v-if="tocNav && toc.length" v-show="tocIsShow">
                ...
                <ul>
                    <li v-for="item of toc" :key="item.id" @click="toTarget(item.id)"
                        :style="{ 'padding-left': item.tag - maxTitle + 'em' }" v-html="item.text">
                    </li>
                </ul>
            </div>
        </transition>
    ...
    methods: {
        LoadArticle() {
            ...
            Post(url, postData).then(res => {
                this.initialValue = res.data.records[4].content
            }).catch(err => {
                console.log(err)
                this.$message({
